...
COMBINED_PLOTS_DIR = os.path.join(os.path.dirname(__file__), "../../figures/plots/poly1305")
...
def plot_one(csv_path: str) -> None:
    ...
    ax.set_title(f"Poly1305 create tag -- Roofline  (-O{opt_level})")
    ax.grid(True, which="both", linestyle=":", alpha=0.6)
    ax.legend(
    loc='upper left',
    bbox_to_anchor=(1.02, 1),
    ...
    ax.set_xlabel("Ciphertext length [bytes]")
    ax.set_ylabel("Performance [ops/cycle]")
    ax.set_title(f"Poly1305 create tag -- ops/cycle vs size  (-O{opt_level})")
    ax.grid(True, which="both", linestyle=":", alpha=0.6)
    ax.legend(
    loc='upper left',
    bbox_to_anchor=(1.02, 1),
