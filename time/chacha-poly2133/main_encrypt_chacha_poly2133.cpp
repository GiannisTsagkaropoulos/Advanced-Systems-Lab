...
void add_function(chacha20_poly2133_encrypt_func f, std::string name);

static std::vector<chacha20_poly2133_encrypt_func> userFuncs;
...
void register_functions() {
    add_function(&chacha20_poly2133_encrypt_baseline, "chacha20_poly2133_encrypt_baseline");
    add_function(&chacha20_poly2133_encrypt_strength_reduction, "chacha20_poly2133_encrypt_strength_reduction");
...
int main(int argc, char* argv[]) {
    ...
    size_t alloc_size    = (PTXT_LEN + TAG_SIZE_2133 + 31) & ~31;
    ...
    chacha20_poly2133_encrypt_baseline(ctxt_base, ptxt, PTXT_LEN, aad, aad_len, key, nonce);
    
    std::function<void(chacha20_poly2133_encrypt_func)> runner = [&](chacha20_poly2133_encrypt_func f) {
    ...
    for (int i = 0; i < numFuncs; i++) {
        ...
        chacha20_poly2133_encrypt_func f = userFuncs[i];
        f(ctxt_test, ptxt, PTXT_LEN, aad, aad_len, key, nonce);

        bool isWrong = (std::memcmp(ctxt_test, ctxt_base, PTXT_LEN + TAG_SIZE_2133) != 0);
        if (isWrong)
